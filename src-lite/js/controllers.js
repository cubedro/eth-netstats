...
netStatsApp.controller("StatsCtrl", function(
    ...
    switch (action) {
      ...
      case "add":
        ...
        if (addNewNode(data))
          toastr["success"](
            "New node " +
              $scope.nodes[findIndex({ id: data.id })].info.name +
              " connected!",
            "New node!"
          );
        else
          toastr["info"](
            "Node " + $scope.nodes[index].info.name + " reconnected!",
            "Node is back!"
          );
      ...
      case "inactive":
        ...
        if (index >= 0) {
          ...
          toastr["error"](
            "Node " + $scope.nodes[index].info.name + " went away!",
    ...
    if ($scope.nodes.length) {
      ...
      _($scope.nodes)
        .map(function(item) {
          ...
          else chains[item.stats.block.number][item.stats.block.fork].count++;

          chains[item.stats.block.number][item.stats.block.fork].score =
            chains[item.stats.block.number][item.stats.block.fork].trusted *
              50 +
  ...
  function forkFilter(node) {
    ...
    if (
      $scope.chains[node.stats.block.number].fork === node.stats.block.fork &&
      $scope.chains[node.stats.block.number].score / $scope.maxScore >= 0.5
    ) {
      node.readable.forkClass = "hidden";
      node.readable.forkMessage = "";
    ...
    if ($scope.chains[node.stats.block.number].score / $scope.maxScore < 0.5) {
      ...
      node.readable.forkMessage =
        "May not be main chain.<br/>Waiting for more confirmations.";
