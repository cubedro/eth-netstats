...
angular
  ...
  .directive("validatorblock", function($compile) {
    return {
      restrict: "E",
      template: "<div></div>",
      replace: true,
            ...
            scope.map = new Datamap({
              ...
              bubblesConfig: {
                ...
                function animadraw() {
                  var x = Math.min(
                    0,
                    Math.max(ev.translate[0], -1 * width * (ev.scale - 1))
                  );
                  var y = Math.min(
                    0,
                    Math.max(ev.translate[1], -1 * height * (ev.scale - 1))
                  );
      ...
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          var color = d3.scale
            ...
            .range(["#3DCF35", "#FF9641", "#c20061", "#FF4500"]);
          ...
          var tip = d3
            ...
            .html(function(d) {
              return (
                '<div class="tooltip-arrow"></div><div class="tooltip-inner"><b>' +
                d.x / 1000 +
                "s - " +
                (d.x + d.dx) / 1000 +
                's</b><div class="small">Percent: <b>' +
                Math.round(d.y * 100) +
                "%</b>" +
                "<br>Frequency: <b>" +
                d.frequency +
                "</b><br>Cumulative: <b>" +
                Math.round(d.cumpercent * 100) +
                "%</b></div></div>"
              );
            });
